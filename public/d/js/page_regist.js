$(document).ready(function(){
	...
	var validate = $("#signupForm").validate({
		...
		messages: {
			email: {
				required: $.i18n.prop("邮箱不能为空"),
				email: $.i18n.prop("邮箱格式不正确")
			},
			password: {
				required: $.i18n.prop("密码不能为空"),
				minlength: jQuery.format($.i18n.prop("密码不少于4位")),
				maxlength: jQuery.format($.i18n.prop("密码不最长16位"))
			},
			passwordAgain: {
				required: $.i18n.prop("确认密码不能为空"),
				minlength: jQuery.format($.i18n.prop("确认密码不少于4位")),
				maxlength: jQuery.format($.i18n.prop("确认密码最长16位")),
				equalTo: jQuery.format($.i18n.prop("密码和确认密码不一致"))
			},
			contact: {
				required: $.i18n.prop("用户名不能为空")
			},
			company: {
				required: $.i18n.prop("年龄不能为空")
			},
			tel: {
				required: $.i18n.prop("手机号不能为空"),
				digits: $.i18n.prop("手机号格式不正确")
			},
			age: {
				required: $.i18n.prop("年龄不能为空")
			}
